...
func isDollar2(curr Curr) bool { 
  dollars := []Curr{currencies[2], currencies[3], currencies[5]} 
  switch curr { 
  default: 
    return false 
  case dollars[0]: 
...
There is no automatic fall through in Go's case clause as there is in the C or Java switch statements. A switch 
block will exit after executing its first matching case. The code must explicitly place the fallthrough keyword, 
as the last statement in a case block, to force the execution flow to fall through the successive case block.
